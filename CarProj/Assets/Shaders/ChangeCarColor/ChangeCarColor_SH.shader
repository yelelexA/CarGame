Shader "car/changeColorSH" {
	Properties {
		[MainTexture] _BaseMap("Base Map (RGB) Smoothness / Alpha (A)", 2D) = "white" {}
		[MainColor]   _BaseColor("Base Color", Color) = (0.7735849, 0.7735849, 0.7735849, 1)
	...
	SubShader {
		...
		Pass {
			...
			#pragma multi_compile _ LIGHTMAP_SHADOW_MIXING // v10+ only => "_MIXED_LIGHTING_SUBTRACTIVE"
			...
			void InitalizeSurfaceData(Varyings IN, out SurfaceData surfaceData){
                ...
                surfaceData.metallic = 1.0; // unused
			...
			void InitializeInputData(Varyings input, half3 normalTS, out InputData inputData) {
				...
				inputData.positionWS = input.positionWS;
					...
					inputData.normalWS = 
                        TransformTangentToWorld(normalTS,half3x3(input.tangentWS.xyz, input.bitangentWS.xyz, input.normalWS.xyz));
				#else
